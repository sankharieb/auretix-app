...
function hasSignal(value) {
  ...
  return Boolean(cleaned && !/not available|unknown|n\/a/i.test(cleaned));
}
...
function sourceStatus(label, available, detail = "") {
  return {
    label,
...
function sourcesForIssue(category, row = {}, confidenceDrivers = []) {
  const isCash = category === "Cash Opportunity" || category === "Procurement Decision";
  const isFlow = category === "Stockout Risk" || category === "Supplier Risk";
  const channelAvailable =
    hasSignal(row.channelAvailability) || hasSignal(row.locationIssue);
  const sources = [
    sourceStatus(
      "Inventory",
      hasSignal(row.inventory) || hasSignal(row.daysOfCover),
      "Inventory on hand and days of cover.",
    ),
    sourceStatus(
      "Sales",
      hasSignal(row.dailyDemand) || hasSignal(row.monthlySales),
      "Sales velocity used for current demand rate.",
    ),
    sourceStatus(
      "Purchase Orders",
      hasSignal(row.openPo) || hasSignal(row.recommendedPo) || hasSignal(row.cashRequired),
      "Open PO, modeled PO quantity, or purchase cash requirement.",
    ),
    sourceStatus(
      "Supplier Records",
      hasSignal(row.supplierReliability) || hasSignal(row.leadTime),
      "Supplier reliability and lead-time assumptions.",
    ),
    sourceStatus(
      "Inbound Schedule",
      hasSignal(row.inboundEtaDays) || hasSignal(row.inboundEtaDate),
      "Inbound ETA used in service-gap projection.",
    ),
    sourceStatus(
      "Decision Outcomes",
      confidenceDrivers.length > 0,
      "Prior decision and outcome history used for confidence adjustment.",
    ),
  ];
  ...
  if (isFlow) {
    sources.push(
      sourceStatus(
        ...
        channelAvailable,
        "Marketplace or location availability used for flow interpretation.",
      ),
      sourceStatus(
        "Carrier Tracking",
        false,
        "Carrier status is not connected yet.",
      ),
    );
  }
  ...
  if (isCash) {
    sources.push(
      sourceStatus(
        "Accounting / Payout Data",
        false,
        "Accounting, payout, or bank-feed data is not connected yet.",
      ),
    );
  }
...
function dataQualityFor(dataCompleteness, confidence, missingCount) {
  const score = dataCompleteness * 0.58 + confidence * 0.42 - missingCount * 2.5;
...
function projectionHealthFor(dataCompleteness, confidence, missingCount) {
  const score = dataCompleteness * 0.55 + confidence * 0.45 - missingCount * 2;

  if (score >= 76) {
    return {
      label: "Strong evidence",
      tone: "green",
      explanation: "Most required operating signals are present and confidence is supported by available data.",
    };
  }
  ...
  if (score >= 52) {
    return {
      ...
      tone: "yellow",
      explanation: "The projection is useful, but missing or inferred inputs should be reviewed before acting.",
    };
  }
  ...
  return {
    ...
    tone: "red",
    explanation: "The projection relies on too many missing or inferred inputs to treat as strong evidence.",
  };
}

function splitConfidenceDrivers(drivers = [], missingSources = []) {
  const supporting = [];
  const reducing = [];

  drivers.forEach((driver) => {
    if (/reduced|below|limited|missing|delay|volatile|variance|neutral/i.test(driver)) {
      reducing.push(driver);
    } else {
      supporting.push(driver);
    }
  });

  if (!supporting.length) {
    supporting.push("Current inventory, demand, supplier, and inbound signals are available for this projection.");
  }

  if (missingSources.length) {
    reducing.push(`Missing inputs currently include ${missingSources.join(", ")}.`);
  }

  return {
    supporting: uniqueItems(supporting).slice(0, 4),
    reducing: uniqueItems(reducing).slice(0, 4),
  };
}

function confidenceExplanationFor({ row = {}, confidenceDrivers = [], missingSources = [] } = {}) {
  const supplierReliability = safeNumber(row.supplierReliability);
  const leadTime = safeNumber(row.leadTime);
  const inboundEtaDays = safeNumber(row.inboundEtaDays);
  const dailyDemand = safeNumber(row.dailyDemand);
  const explanations = [];
  ...
  explanations.push({
    ...
    detail:
      confidenceDrivers.find((driver) => /historical|outcome|accurate/i.test(driver)) ||
      "Outcome-backed forecast accuracy is still building, so this input is treated cautiously.",
  });

  explanations.push({
    label: "Supplier reliability impact",
    detail:
      supplierReliability > 0
        ? `Supplier reliability is currently ${Math.round(supplierReliability)}%, which affects timing confidence.`
        : "Supplier reliability is not connected, reducing the strength of the projection.",
  });

  explanations.push({
    label: "Demand volatility impact",
    detail:
      dailyDemand > 0
        ? `Demand uses the current velocity signal of ${dailyDemand.toFixed(1)} units per day.`
        : "Demand volatility is inferred until live sales history is connected.",
  });

  explanations.push({
    label: "Lead-time variability impact",
    detail:
      leadTime > 0 || inboundEtaDays > 0
        ? `Lead-time context uses ${leadTime || inboundEtaDays} days and inbound timing where available.`
        : "Lead-time variability is inferred because live supplier or carrier timing is missing.",
  });

  explanations.push({
    label: "Missing data impact",
    detail: missingSources.length
      ? `Missing data lowers trust: ${missingSources.join(", ")}.`
      : "No major required source is missing for this projection.",
  });
...
function buildTrustProfile({
  category = "Forecast Concern",
  row = {},
  confidence = 0,
  confidenceDrivers = [],
  evidence = [],
} = {}) {
  const sources = sourcesForIssue(category, row, confidenceDrivers);
  const connectedSources = sources.filter((source) => source.available);
  const missingSources = sources.filter((source) => !source.available);
  const dataCompleteness = Math.round((connectedSources.length / Math.max(1, sources.length)) * 100);
  const realDataPercent = dataCompleteness;
  const inferredDataPercent = clamp(100 - realDataPercent);
  const modelConfidence = Math.round(clamp(confidence || 0));
  const dataQuality = dataQualityFor(dataCompleteness, modelConfidence, missingSources.length);
  const projectionHealth = projectionHealthFor(
    dataCompleteness,
    modelConfidence,
    missingSources.length,
  );
  const driverSplit = splitConfidenceDrivers(
    confidenceDrivers.length ? confidenceDrivers : evidence,
    missingSources.map((source) => source.label),
  );
  ...
  return {
    ...
    dataQuality,
    projectionHealth,
    confidenceDrivers: driverSplit,
    confidenceExplanation: confidenceExplanationFor({
      row,
      confidenceDrivers,
...
function combineTrustProfiles(profiles = []) {
  const usable = profiles.filter(Boolean);

  if (!usable.length) {
    return buildTrustProfile({
      confidence: 45,
      confidenceDrivers: ["Trust is limited until more operating data is connected."],
    });
  }

  const dataCompleteness = Math.round(
    average(usable.map((profile) => profile.dataCompleteness)),
  );
  const modelConfidence = Math.round(
    average(usable.map((profile) => profile.modelConfidence)),
  );
  const connectedLabels = uniqueItems(
    usable.flatMap((profile) => profile.connectedSources?.map((source) => source.label) || []),
  );
  const missingLabels = uniqueItems(
    usable.flatMap((profile) => profile.missingSources?.map((source) => source.label) || []),
  ).filter((label) => !connectedLabels.includes(label));
  const dataQuality = dataQualityFor(dataCompleteness, modelConfidence, missingLabels.length);
  const projectionHealth = projectionHealthFor(
    dataCompleteness,
    modelConfidence,
    missingLabels.length,
  );
  const supporting = uniqueItems(
    usable.flatMap((profile) => profile.confidenceDrivers?.supporting || []),
  ).slice(0, 4);
  const reducing = uniqueItems(
    usable.flatMap((profile) => profile.confidenceDrivers?.reducing || []),
  ).slice(0, 4);

  return {
    dataCompleteness,
    connectedSources: connectedLabels.map((label) => sourceStatus(label, true)),
    missingSources: missingLabels.map((label) => sourceStatus(label, false)),
    realDataPercent: dataCompleteness,
    inferredDataPercent: clamp(100 - dataCompleteness),
    modelConfidence,
    dataQuality,
    projectionHealth,
    confidenceDrivers: { supporting, reducing },
    confidenceExplanation: [
      {
        label: "Historical forecast accuracy",
        detail: "Aggregate trust combines the available confidence explanations across visible projections.",
      },
      {
        label: "Missing data impact",
        detail: missingLabels.length
          ? `Missing aggregate inputs include ${missingLabels.join(", ")}.`
          : "No major aggregate input is missing.",
      },
    ],
  };
}
...
function buildEvidenceDrilldown(issue, responsePaths = []) {
  ...
  return {
    ...
    trust: buildTrustProfile({
      ...
      row,
      confidence,
      confidenceDrivers: confidenceReasoning,
      evidence,
    }),
  };
}
...
function buildGroupedStockoutEvidenceDrilldown(issues = [], responsePaths = []) {
  ...
  return buildFallbackEvidenceDrilldown({
    ...
    responsePaths,
    category: "Stockout Risk",
    trustProfile: combineTrustProfiles(
      stockoutIssues.map((issue) => issue.evidenceDrilldown?.trust),
    ),
  });
}
...
export function buildAuretixAdvisorCommandCenter(options = {}) {
  ...
  const aggregateTrust = combineTrustProfiles(
    advisorProblems.map((problem) => problem.evidenceDrilldown?.trust),
  );
