...
function evidenceFor(category, recommendation, row = {}) {
  ...
  if (safeNumber(row.recommendedPo) > 0) {
    evidence.push(`Potential buy quantity is ${integer(row.recommendedPo)} units.`);
  }
...
function confidenceReasoningFor(recommendation) {
  ...
  if (historicalAdjustment > 0) {
    reasoning.push("Historical decision performance increased confidence.");
  } else if (historicalAdjustment < 0) {
    reasoning.push("Historical decision performance reduced confidence.");
  } else {
    reasoning.push("Historical decision performance is still neutral.");
  }
...
function nextStepFor(category, recommendation) {
  ...
  if (partnerSupport.available) {
    return `Review the evidence, then choose whether to request ${String(partnerSupport.label || "partner help").toLowerCase()}.`;
  }

  if (category === "Procurement Decision") {
    return "Review the PO evidence, then choose approve, defer, or watch.";
  }

  if (category === "Stockout Risk") {
    return "Review the evidence and compare acceleration, split, partner help, or monitor paths.";
  }

  return "Review the evidence and choose approve, defer, or watch.";
}
...
function buildBriefingItem(recommendation, row, index) {
  ...
  return {
    ...
    recommendedAction: recommendation.recommendedAction || recommendation.riskIndex?.recommendedAction || "Review current evidence",
    reasonIntro: "Why this signal appears:",
    ...
    rowSnapshot: {
      inventory: safeNumber(row.inventory),
      dailyDemand: safeNumber(row.dailyDemand),
      daysOfCover: safeNumber(row.daysOfCover),
      inboundEtaDays: safeNumber(row.inboundEtaDays),
      inboundEtaDate: row.inboundEtaDate || null,
      serviceGapDays: safeNumber(row.serviceGapDays),
      monthlySales: safeNumber(row.monthlySales),
      openPo: safeNumber(row.openPo),
      leadTime: safeNumber(row.leadTime),
      supplierReliability: supplierReliability(recommendation, row),
      recommendedPo: safeNumber(row.recommendedPo),
      cashRequired: safeNumber(row.cashRequired),
      revenueAtRisk: safeNumber(row.revenueAtRisk),
      cashTrapped: safeNumber(row.cashTrapped),
      unitPrice: safeNumber(row.unitPrice),
      unitCost: safeNumber(row.unitCost),
      inboundDelayRisk: safeNumber(row.inboundDelayRisk),
      channelAvailability: row.channelAvailability || "Not available",
      locationIssue: row.locationIssue || "Not available",
      recommendedMove: row.recommendedMove || recommendation.recommendedMove || "",
    },
    profitImpact: recommendation.profitImpact || {},
    riskIndex: recommendation.riskIndex || {},
    partnerSupport: recommendation.partnerSupport || recommendation.riskIndex?.partnerSupport || {},
  };
}
...
export function buildDailyExecutiveBriefing({
  ...
  return {
    ...
    summary: items.length
      ? `${items.length} notable ${issueWord} were detected in the current data.`
      : "No major operational changes were detected in the current data.",
