...
import { loadModelGuidanceRules } from "./moat-model-guidance-store.js";
...
function fromDbProfitImpactRecord(row) {
  if (!row) {
    return null;
  }
  ...
  return {
    ...
    marginAtRisk: row.margin_at_risk,
    cashTiedUp: row.cash_tied_up,
    potentialStockoutLoss: row.potential_stockout_loss,
    overstockExposure: row.overstock_exposure,
...
async function getSupabaseDecisionHistory(supabase, auth, workspaceId) {
  ...
  if (riskScoresError) {
    throw riskScoresError;
  }

  const { data: profitImpactRecords, error: profitImpactRecordsError } = await supabase
  ...
  if (profitImpactRecordsError) {
    throw profitImpactRecordsError;
  }
  ...
  return {
    ...
    riskScores: (riskScores || []).map(fromDbRiskScore),
    profitImpactRecords: (profitImpactRecords || []).map(fromDbProfitImpactRecord),
    auditEvents: (auditEvents || []).map(fromDbAuditEvent),
  };
}
...
function buildBundleFromHistory(snapshot, history, guidanceBundle = {}, context = {}) {
  const modelGuidanceRules = Array.isArray(guidanceBundle.rules) ? guidanceBundle.rules : [];
  const learningAnalytics = buildLearningAnalytics({
    decisionRecommendations: history.decisions,
    decisionOutcomes: history.outcomes,
    riskScores: history.riskScores,
    profitImpactRecords: history.profitImpactRecords,
  ...
  const dailyBriefing = buildDailyExecutiveBriefing({
    ...
    rows: snapshot.rows,
    ownerName: context.user?.name || context.user?.email,
  });

  return {
    ...snapshot,
    recommendations: confidenceBundle.recommendations,
    dailyDecisionQueue: confidenceBundle.recommendations,
    dailyBriefing,
    ...
    modelGuidanceRules,
    humanGovernance: guidanceBundle.humanGovernance || {
      pendingRules: 0,
      activeRules: 0,
      rejectedRules: 0,
      totalApprovedAdjustments: 0,
      averageApprovedAdjustment: 0,
      guidanceRulesInfluencingConfidence: 0,
    },
    guidanceMigrationRequired: Boolean(guidanceBundle.migrationRequired),
  };
}
...
export async function getMoatEngineBundle(workspaceId = "workspace_demo", context = {}) {
  ...
  if (supabase) {
    try {
      ...
      const guidanceBundle = await loadModelGuidanceRules(resolvedWorkspaceId, auth, {
        supabase,
        skipTenantEnsure: true,
      });
      const snapshot = buildMoatEngineSnapshot({
        decisionHistory: history.decisions,
        decisionOutcomes: history.outcomes,
      });
      const bundle = buildBundleFromHistory(snapshot, history, guidanceBundle, auth);
  ...
  if (supabase) {
    try {
      ...
      await appendSupabaseAudit(supabase, {
        ...
        detail: `${dbOutcome.accuracyStatus} outcome recorded for ${dbOutcome.sku}; actual impact ${dbOutcome.actualFinancialImpact}; recommendation ${dbOutcome.recommendationId}.`,
      });
